...
- **图片查看**：MCP 可把聊天图片和表情包交给 AI 理解；新版 V2 加密图片配置 `image_aes_key` 后可解密

> 注意：2026 年 3 月后微信更新了图片存储加密方式，新图片是 V2 加密 `.dat`。本项目已支持用 `image_aes_key` 解密，但这个 key 需要在微信查看图片时临时捕获；未配置 key 时，新图片只能定位文件，不能交给 AI 查看。

如果要读取新版 V2 图片，可以在微信里打开几张近期图片，同时运行 `sudo .venv/bin/python3 monitor_v2_key.py` 尝试捕获图片 AES key。捕获成功后会写入 `~/.wechat-summary/config.json` 的 `image_aes_key` 字段，MCP 的 `get_chat_images` 会自动用它解密。

### 关注推送与 Obsidian 知识库

- **关注推送**：后台监控指定群聊，出现你关心的新功能、链接、教程、实验结果、八卦等内容时通知你
- **链接展开**：关注推送会尝试读取公开网页标题和摘要，也会从本地微信 XML 展开转发聊天记录；只有只剩私有网页壳时才标记正文不可见
...
关注推送会在判断前尝试展开消息里的公开网页链接，给 AI 补充标题和摘要。微信转发聊天记录如果在本地消息 XML 里带有条目，会直接解析成可读摘要；如果只剩私有网页壳，程序会标记为「正文不可见」，再结合群聊上下文判断是否提醒。
