...
- **图片查看** — MCP 可把聊天图片和表情包交给 AI 查看；新版 V2 加密图片配置 `image_aes_key` 后可解密
...
读取新版 V2 图片时，可以在微信里打开几张近期图片，同时运行 `sudo .venv/bin/python3 monitor_v2_key.py` 尝试捕获图片 AES key。成功后会写入 `~/.wechat-summary/config.json` 的 `image_aes_key`，MCP 的 `get_chat_images` 会自动解密。
